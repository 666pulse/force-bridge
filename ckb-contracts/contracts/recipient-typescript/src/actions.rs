use ckb_std::{ckb_constants::Source, ckb_types::prelude::Entity};
...
use force_bridge_types::{
    basic, force_bridge_lockscript::ForceBridgeLockscriptArgs, recipient_cell::RecipientDataView
};
use molecule::prelude::{Builder, Byte};

use crate::sudt::get_sudt_amount_from_source;

pub const CKB_HASH_PERSONALIZATION: &[u8] = b"ckb-default-hash";

pub fn verify_burn_token(data: RecipientDataView) {
    if data.amount == 0 {
        panic!(
    ...
    let input_sudt_num =
        get_sudt_amount_from_source(Source::Input, &force_bridge_lock_hash);
    let output_sudt_num =
        get_sudt_amount_from_source(Source::Output, &force_bridge_lock_hash);
