...
${FORCE_BRIDGE_CLI} eth balanceOf -addr ${CKB_RECIPIENT_ADDRESS}
#
#SOL_LOCKED_ACCOUNT=alice
#SOL_LOCKED_PRIVATE_KEY=5KQG4541B1FtDC11gu3NrErWniqTaPHBpmikSztnX8m36sK5px5
#
#${FORCE_BRIDGE_CLI} sol balanceOf -addr ${CKB_RECIPIENT_ADDRESS} -s SOL
#${FORCE_BRIDGE_CLI} sol lock -p ${SOL_LOCKED_PRIVATE_KEY} -acc ${SOL_LOCKED_ACCOUNT} -r ${CKB_RECIPIENT_ADDRESS} -a '0.0001 SOL'
#sleep 60
#${FORCE_BRIDGE_CLI} sol balanceOf -addr ${CKB_RECIPIENT_ADDRESS} -s SOL
#${FORCE_BRIDGE_CLI} sol unlock -p ${CKB_PRIVATE_KEY} -a '0.0001 SOL' -r ${SOL_LOCKED_ACCOUNT}
#sleep 60
#${FORCE_BRIDGE_CLI} sol balanceOf -addr ${CKB_RECIPIENT_ADDRESS} -s SOL
#
